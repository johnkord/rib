...
pub trait BoardRepo: Send + Sync {
    ...
    async fn get_board(&self, id: Id) -> RepoResult<Board>; // NEW
}
...
pub mod pg {
    ...
    impl BoardRepo for PgRepo {
        ...
        async fn get_board(&self, id: Id) -> RepoResult<Board> {
            let rec = sqlx::query_as::<_, Board>("SELECT id, slug, title, created_at, deleted_at FROM boards WHERE id=$1")
                .bind(id)
                .fetch_one(&self.pool).await.map_err(|_| RepoError::NotFound)?;
            Ok(rec)
        }
        async fn soft_delete_board(&self, id: Id) -> RepoResult<()> {
            let res = sqlx::query("UPDATE boards SET deleted_at = COALESCE(deleted_at, now()) WHERE id=$1")
                .bind(id)
                .execute(&self.pool).await.map_err(|_| RepoError::NotFound)?;
    ...
    impl ThreadRepo for PgRepo {
        async fn list_threads(&self, board_id: Id, include_deleted: bool) -> RepoResult<Vec<Thread>> {
            let base = r#"
                ...
                WHERE t.board_id = $1
            "#;
            let sql = if include_deleted { format!("{base} ORDER BY t.bump_time DESC") } else { format!("{base} AND t.deleted_at IS NULL ORDER BY t.bump_time DESC") };
        ...
        async fn create_thread(&self, new: NewThread) -> RepoResult<Thread> {
            ...
            let rec = sqlx::query(
                "INSERT INTO threads (board_id, subject, body) VALUES ($1,$2,$3) RETURNING id"
            )
                .bind(new.board_id)
                .bind(&new.subject)
                .bind(&new.body)
        ...
        async fn soft_delete_thread(&self, id: Id) -> RepoResult<()> {
            let res = sqlx::query("UPDATE threads SET deleted_at = COALESCE(deleted_at, now()) WHERE id=$1")
                .bind(id).execute(&self.pool).await.map_err(|_| RepoError::NotFound)?;
            if res.rows_affected()==0 { return Err(RepoError::NotFound); }
            Ok(())
        }
        async fn restore_thread(&self, id: Id) -> RepoResult<()> {
    ...
    impl ReplyRepo for PgRepo {
        async fn list_replies(&self, thread_id: Id, include_deleted: bool) -> RepoResult<Vec<Reply>> {
            let base = r#"
                SELECT r.id, r.thread_id, r.content, img.hash as image_hash, img.mime as mime, r.created_at, r.deleted_at
                FROM replies r
                LEFT JOIN LATERAL (
                ...
                WHERE r.thread_id = $1
            "#;
            let sql = if include_deleted { format!("{base} ORDER BY r.created_at ASC") } else { format!("{base} AND r.deleted_at IS NULL ORDER BY r.created_at ASC") };
        ...
        async fn create_reply(&self, new: NewReply) -> RepoResult<Reply> {
            ...
            let rec = sqlx::query(
                "INSERT INTO replies (thread_id, content) VALUES ($1,$2) RETURNING id"
            )
                .bind(new.thread_id)
                .bind(&new.content)
                .fetch_one(&mut *tx)
        ...
        async fn soft_delete_reply(&self, id: Id) -> RepoResult<()> {
            let res = sqlx::query("UPDATE replies SET deleted_at = COALESCE(deleted_at, now()) WHERE id=$1")
                .bind(id).execute(&self.pool).await.map_err(|_| RepoError::NotFound)?;
            if res.rows_affected()==0 { return Err(RepoError::NotFound); }
            Ok(())
        }
    ...
    impl DiscordRoleRepo for PgRepo {
        async fn get_discord_user_role(&self, _discord_id: &str) -> Option<AuthRole> {
            None
        }
        async fn set_discord_user_role(
            &self,
            _discord_id: &str,
            _role: AuthRole,
        ) -> RepoResult<()> {
