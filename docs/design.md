...
## 17. Browser Frontend (rib-react)

### 17.1 Overview  
A single-page application (SPA) that consumes the `/api/v1` REST endpoints provided by RIB. It lives in the `rib-react` directory within the main repository, providing a React-based user interface for the image board.
...
• HTTP: `@tanstack/react-query` for request caching & deduplication  
• Auth: JWT (stored in localStorage) with custom hooks  
• Build: Vite (fast builds, HMR), ESLint + Prettier  
• Testing: Vitest + React Testing Library

### 17.3 Core Pages  
| Route | Description |
|-------|-------------|
| `/` | Global board list (BoardsPage) |
| `/b/{slug}` | Thread catalog for a board (BoardThreadsPage) |
| `/thread/{id}` | Thread view with replies (ThreadPage) |
| `/login` | Discord OAuth login page |
| `/admin/roles` | Admin-only Discord role management |

### 17.4 Components  
- `Navbar` – Global navigation with auth status and logout  
- `MediaModal` – Full-screen image/video viewer with navigation  
- `BoardsPage` – Board creation and listing  
- `ThreadPage` – Thread view with inline reply form  
- `AdminRoles` – Discord user role assignment (admin-only)

### 17.5 API Integration  
All requests use a centralized `fetchJson` wrapper that:
...
### 17.6 Deployment  
`rib-react` is built into static assets (`dist/`) and either:  
...
3. Served separately via Vite dev server for development (port 5173)

### 17.7 Accessibility & i18n  
- WCAG 2.1 AA compliance baseline  
- Semantic HTML with proper ARIA labels
- Internationalization planned (react-i18n in future)

### 17.8 Roadmap  
Phase 1 (v0.1) – ✅ read-only pages, board/thread listing  
Phase 2 (v0.5) – ✅ posting flows, image/video upload, Discord auth  
Phase 3 (v1.0) – moderation UI, live updates via WS, search