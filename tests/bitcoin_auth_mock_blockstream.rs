...
async fn bitcoin_auth_mocked_blockstream_balance_allows_login() {
    ...
    let utxos = serde_json::json!([
        {"txid":"abcd","vout":0,"status":{"confirmed":true},"value":1000000},
        {"txid":"efgh","vout":1,"status":{"confirmed":true},"value":500000}
    ]);
    ...
    assert_eq!(
        resp.status(),
        200,
        "verify should succeed with mocked sufficient balance"
    );
