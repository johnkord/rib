...
fn user_token() -> String {
    ensure_secret();
    create_jwt("user", "user", vec![Role::User]).unwrap()
}
...
async fn rate_limit_thread_creation() {
    ...
    let cfg = RateLimitConfig {
        thread_limit: 1,
        thread_window: std::time::Duration::from_secs(300),
        reply_limit: 100,
        reply_window: std::time::Duration::from_secs(60),
        image_limit: 100,
        image_window: std::time::Duration::from_secs(3600),
    };
    ...
    let req = test::TestRequest::post()
        ...
        .set_json(json!({"slug":slug, "title":"RL"}))
        .to_request();
    ...
    let req = test::TestRequest::post()
        ...
        .set_json(json!({"board_id":board.id, "subject":"S1", "body":"B1"}))
        .to_request();
    let resp = test::call_service(&app, req).await;
    ...
    let req = test::TestRequest::post()
        .uri("/api/v1/threads")
        .insert_header(("Authorization", format!("Bearer {user}")))
        .set_json(json!({"board_id":board.id, "subject":"S2", "body":"B2"}))
        .to_request();
    let resp = test::call_service(&app, req).await;
