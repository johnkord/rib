...
export function isSoundCloud(url: string): boolean {
  return /https?:\/\/(?:on\.)?soundcloud\.com\//.test(url);
}
...
export function createVideoEmbed(
  ...
  return React.createElement(
    'div',
    { key: `video-${embed.videoId}`, className: 'video-embed-container mb-4' },
    [
      ...
      React.createElement(
        'div',
        { key: `link-${embed.videoId}`, className: 'text-xs text-gray-500 mt-1' },
        [
          React.createElement(
            'a',
            {
              key: `anchor-${embed.videoId}`,
              href: originalUrl,
              target: '_blank',
