...
export function BoardThreadsPage() {
  ...
  } = useThreads(boardId, user?.role === 'admin' && showDeleted); // include deleted
    ...
    <div>
      ...
      <div className="flex items-center justify-between mb-3">
        <div className="flex items-center gap-2">
          <h1 className="text-xl font-semibold">/{slug}/ - Threads</h1>
          <button className="btn btn-sm" onClick={() => refreshThreads()} disabled={isFetching}>
      ...
      <ul>
        ...
        {!isFetching &&
          sortedThreads.map((t) => (
            <li
              key={t.id}
              className={`mb-1 flex items-center gap-2 ${t.deleted_at ? 'opacity-60' : ''}`}
            >
              ...
              {t.image_hash &&
                (t.mime?.startsWith('image/') ? (
                  <img
                    className="inline-block h-6 ml-2 cursor-pointer"
                    src={imageUrl(t.image_hash)}
                    alt=""
                ...
                <div className="flex items-center gap-1 ml-2">
                  {!t.deleted_at && (
                    <button
                      ...
                      onClick={async () => {
                        await apiClient.softDelete('threads', t.id);
                        refreshThreads();
                      }}
                    >
                      Soft
                  ...
                  {t.deleted_at && (
                    <button
                      className="btn btn-xs"
                      onClick={async () => {
                        await apiClient.restore('threads', t.id);
                        refreshThreads();
                      }}
                    >
                      Restore
                  ...
                  {user.role === 'admin' && (
                    <button
                      ...
                      onClick={async () => {
                        if (confirm('Hard delete thread? This cannot be undone.')) {
                          await apiClient.hardDelete('threads', t.id);
                          refreshThreads();
                        }
                      }}
                    >
      ...
      {/* media viewer -------------------------------------------------- */}
      {viewer && (
        <MediaModal hash={viewer.hash} mime={viewer.mime} onClose={() => setViewer(null)} />
      )}
      {/* --------------------------------------------------------------- */}
    </div>
  );
