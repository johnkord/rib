          ...
          <li key={t.id} className={`mb-1 flex items-center gap-2 ${t.deleted_at ? 'opacity-60' : ''}`}>
            ...
            {user && (user.role === 'admin' || user.role === 'moderator') && (
              <div className="flex items-center gap-1 ml-2">
                ...
                {user.role === 'admin' && <button className="btn btn-xs btn-error" onClick={async()=>{ if(confirm('Hard delete thread? This cannot be undone.')) { await apiClient.hardDelete('threads', t.id); refreshThreads(); } }}>Hard</button>}
              </div>
            )}
          </li>
        ))}
