...
export function ThreadPage() {
  ...
  const thread = useQuery<Thread>({
    ...
    queryFn: () =>
      fetchJson(`/threads/${threadId}${user?.role === 'admin' ? '?include_deleted=1' : ''}`),
    enabled: !!threadId,
  });
  ...
  } = useReplies(threadId, user?.role === 'admin' && showDeleted);
  ...
  const mediaItems = useMemo<MediaItem[]>(() => {
    ...
    if (thread.data?.image_hash)
      items.push({ hash: thread.data.image_hash, mime: thread.data.mime ?? null });
    sortedReplies.forEach((r) => {
      if (r.image_hash) items.push({ hash: r.image_hash, mime: r.mime ?? null });
    });
    return items;
  ...
  return (
    <div>
      ...
      {boardSlug && (
        <p className="mb-2">
          <Link className="link" to={`/${boardSlug}`}>
      ...
      {!thread.isFetching && thread.data && (
        <>
              ...
              <div className="alert alert-warning mb-3 py-2 px-3 text-sm flex items-center justify-between">
                ...
                <div className="flex gap-2">
                  <button
                    ...
                    onClick={async () => {
                      await apiClient.restore('threads', thread.data!.id);
                  ...
                  <button
                    ...
                    onClick={async () => {
                      if (confirm('Hard delete forever?')) {
                        await apiClient.hardDelete('threads', thread.data!.id);
            ...
            !thread.data.deleted_at && (
              <div className="mb-3 flex gap-2">
                <button
                  className="btn btn-xs"
                  onClick={async () => {
                    await apiClient.softDelete('threads', thread.data!.id);
                    await thread.refetch();
                  }}
                >
                  Soft Delete
                ...
                <button
                  className="btn btn-xs btn-error"
                  onClick={async () => {
                    if (confirm('Hard delete thread permanently?')) {
                      await apiClient.hardDelete('threads', thread.data!.id);
                      window.location.href = '/';
                    }
                  }}
          ...
          <div
            className={`mb-4 whitespace-pre-wrap ${thread.data.deleted_at ? 'opacity-60 line-through' : ''}`}
          >
            ...
            return (
              ...
              >
                <div className="mb-1 text-xs text-gray-500 flex items-center gap-2">
                  <Link className="link" to={`/thread/${threadId}#p${r.id}`}>
                    No.{r.id}
                  </Link>
                    ...
                    <div className="ml-auto flex gap-1">
                      {!deleted && (
                        <button
                          ...
                          onClick={async () => {
                            await apiClient.softDelete('replies', r.id);
                      ...
                      {deleted && (
                        <button
                          className="btn btn-ghost btn-xs"
                          onClick={async () => {
                            await apiClient.restore('replies', r.id);
                            await refreshReplies();
                          }}
                        >
                          Restore
                      ...
                      {user.role === 'admin' && (
                        <button
                          ...
                          onClick={async () => {
                            if (confirm('Hard delete this reply permanently?')) {
                              await apiClient.hardDelete('replies', r.id);
                              await refreshReplies();
                            }
                          }}
                        >
      ...
      {viewer && (
        <MediaModal
          ...
          onPrev={
            viewer.index > 0 ? () => setViewer((v) => v && { ...v, index: v.index - 1 }) : undefined
          }
          onNext={
            viewer.index < viewer.items.length - 1
              ? () => setViewer((v) => v && { ...v, index: v.index + 1 })
              : undefined
          }
